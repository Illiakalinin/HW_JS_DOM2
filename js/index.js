...
news.forEach((n) => {
  const newsListItemEl = createNewsListItem(n);
  newsListEl.append(newsListItemEl);
});

function createNewsListItem({ title, headerBgSrc, category, body, date }) {
  const newsListItemEl = document.createElement("li");
  newsListItemEl.classList.add("newsListItem");
  newsListItemEl.onclick = (e) => {
    e.currentTarget.style.backgroundColor = "khaki";
  };

  const newsTitleEl = document.createElement("article");
  newsTitleEl.classList.add("newsTitle");
  newsListItemEl.append(newsTitleEl);

  const newsTitleBgEl = document.createElement("div");
  newsTitleBgEl.classList.add("newsTitleBg");
  // newsTitleBgEl.style.backgroundImage =
  // "url(https://cdn.mos.cms.futurecdn.net/yS8AdDkWFHzDhEjycLwkc9-970-80.jpg.webp)";
  // newsTitleBgEl.src = headerBgSrc;
  // newsTitleBgEl.alt = category;
  newsTitleEl.append(newsTitleBgEl, createBgContent(title));

  const likeNewsBtn = document.createElement("button");
  likeNewsBtn.innerHTML = '<i class="fas fa-heart"></i>';
  likeNewsBtn.addEventListener("click", (e) => {
    e.currentTarget.style.color = "OrangeRed";
    e.stopPropagation();
  });
  newsTitleBgEl.append(likeNewsBtn);

  const footerEl = document.createElement("footer");
  footerEl.classList.add("dateDelete");
  newsListItemEl.append(createCategory(category), createNews(body), footerEl);

  const removeNewsBtn = document.createElement("button");
  removeNewsBtn.innerHTML = '<i class="fas fa-trash"></i>';
  removeNewsBtn.onclick = (e) => {
    e.target.closest(".newsListItem").remove();
  };
  footerEl.append(createNewsDate(date), removeNewsBtn);
  return newsListItemEl;
}

function createBgContent(title) {
  const bgContentEl = document.createElement("p");
  bgContentEl.classList.add("bgContent");
  bgContentEl.textContent = title;
  return bgContentEl;
}
